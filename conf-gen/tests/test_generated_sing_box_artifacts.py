...
def test_generated_artifacts_pass_schema_and_sing_box_check(
    ...
    android_check_config_names: frozenset[str],
) -> None:
    ...
    secret-redacted copy for config names selected by the CI runner. Android
    configs selected via `--check-config-android` must fail the raw check on
    exactly the Android-only fields and pass once those fields are stripped.
    This test does not generate configs or start sing-box at runtime; those
    paths are covered by the source-derived pytest cases.
    """
    ...
    checked_config_names: set[str] = set()
    with rule_set_compiler() as compiler:
        for config_dir in config_dirs:
            ...
            if config_dir.name in check_config_names:
                ...
                checked_config_names.add(config_dir.name)
            if config_dir.name in android_check_config_names:
                run_android_filtered_sing_box_check(compiler._sing_box, config_dir, config)
                checked_config_names.add(config_dir.name)

    expected_config_names = check_config_names | android_check_config_names
    if checked_config_names != expected_config_names:
        missing_config_names = sorted(expected_config_names - checked_config_names)
