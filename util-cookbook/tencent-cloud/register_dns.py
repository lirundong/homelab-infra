...
def get_qcloud_signature(
    ...
    request_plain_str = f"{request_method}{QCLOUD_API_HOSTNAME}{QCLOUD_API_PATH}?" + "&".join(
        ordered_request
    )
    signature = hmac.new(
...
def get_full_request(
    ...
    qcloud_secret_id, qcloud_secret_key = (
        secrets.QCLOUD_API_SECRET_ID,
        secrets.QCLOUD_API_SECRET_KEY,
    )
    ...
    request["Signature"] = get_qcloud_signature(
        request, qcloud_secret_key, request_method, hmac_method
    )
...
def get_qcloud_subdomain_record(
    ...
    response = requests.get(
        f"https://{QCLOUD_API_HOSTNAME}{QCLOUD_API_PATH}", params=request
    ).json()
